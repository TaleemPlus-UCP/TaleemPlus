...
class _TeacherQuizListScreenState extends State<TeacherQuizListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (user != null) {
        context.read<ClassProvider>().listenAll(); // Standard method for loading classes
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.isAiGen ? 'AI Test Generator' : 'Grading & Marks Entry', 
  ...
  Widget _buildNoClassState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.class_outlined, size: 64, color: AppColors.textMuted.withValues(alpha: 0.3)),
  ...
  Widget _buildQuizList(String classId) {
    return StreamBuilder<List<QuizModel>>(
      ...
      builder: (context, snap) {
        ...
        if (list.isEmpty) {
          return const Center(child: Text('No tests created yet', style: TextStyle(color: AppColors.textSecondary)));
        }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text("${quiz.subject} • ${quiz.month} • ${quiz.totalMarks} Marks", style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
                  ],
                ),
              ),
              IconButton(
              ...
              if (widget.isAiGen)
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () => PdfGeneratorService.printTestPaper(quiz),
                    icon: const Icon(Icons.print_rounded, size: 18),
                    label: const Text("PRINT PAPER"),
              ...
              if (!widget.isAiGen)
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => MarkEntryScreen(quiz: quiz)),
                    ),
  ...
  Future<void> _confirmDelete(QuizModel quiz) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete Test?', style: TextStyle(color: AppColors.textPrimary)),
        content: const Text('This will remove the test and all student marks permanently.', style: TextStyle(color: AppColors.textSecondary)),
        actions: [
